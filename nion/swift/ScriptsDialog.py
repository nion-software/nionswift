...
class ScriptListCanvasItemDelegate(Widgets.ListCanvasItemDelegate):
    ...
    @staticmethod
    def _closed_folder_icon_string() -> str:
        if sys.platform == "win32":
            triangle = "\N{BLACK MEDIUM RIGHT-POINTING TRIANGLE CENTRED} "
        else:
            triangle = f"\N{BLACK DOWN-POINTING TRIANGLE} "
        return triangle + f"\N{FILE FOLDER} "

    @staticmethod
    def _open_folder_icon_string() -> str:
        if sys.platform == "win32":
    ...
    def mouse_pressed_in_item(self, mouse_index: int, pos: Geometry.IntPoint, modifiers) -> bool:
        ...
        width = self.__ui.get_font_metrics(self._major_font_size(), self._closed_folder_icon_string()).width
        if isinstance(display_item, FolderListItem) and display_item.indent - 4 < pos.x < display_item.indent + width - 20:
    ...
    def __calculate_indent(self, display_item):
        ...
        icon_string = self._open_folder_icon_string()
        return 4 + self.__ui.get_font_metrics(self._major_font_size(), icon_string).width * display_item.indent_level

    def paint_item(self, drawing_context, display_item, rect, is_selected):
        if isinstance(display_item, FolderListItem):
            if display_item.folder_closed:
                icon_string = self._closed_folder_icon_string()
            else:
                icon_string = self._open_folder_icon_string()
        else:
        ...
        if isinstance(display_item, ScriptListItem):
            with drawing_context.saver():
                ...
                name_string = icon_string + display_item.basename
                display_item.indent = self.__calculate_indent(display_item)
                drawing_context.fill_text(name_string, rect[0][1] + display_item.indent, rect[0][0] + 20 - 4)
