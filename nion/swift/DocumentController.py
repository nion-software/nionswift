...
class AssignVariableReference(Window.Action):
    ...
    def invoke(self, context: Window.ActionContext) -> Window.ActionResult:
        ...
        data_item = context.data_item
        if display_item:
            data_item_var, display_item_var = context.model.assign_variable_to_display_item(display_item)
            logging.debug("{} = Display Item with UUID {}".format(display_item_var, display_item.uuid))
            if data_item:
                logging.debug("{} = Data Item with UUID {}".format(data_item_var, data_item.uuid))
            for console in window.consoles:
                console.assign_item_var(display_item_var, display_item)
                if data_item:
                    console.assign_item_var(data_item_var, data_item)
        return Window.ActionResult.FINISHED
