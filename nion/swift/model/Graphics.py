...
class DrawingContextLike(typing.Protocol):
    ...
    line_width: float
    line_dash: typing.Optional[float]
...
def draw_marker(ctx: DrawingContextLike, p: Geometry.FloatPoint, is_enabled: bool, is_locked: bool) -> None:
    if not is_locked:
        with ctx.saver():
            marker_color = '#00FF00'
            ctx.begin_path()
...
def draw_centre_marker(ctx: DrawingContextLike, origin: Geometry.FloatPoint, graphic_size: Geometry.FloatSizeTuple, stroke_style: typing.Any, is_move_locked: bool, mark_size: int = 8) -> None:
    mid_x = origin[1]
    mid_y = origin[0]
    if graphic_size[0] > mark_size and graphic_size[1] > mark_size and is_move_locked:
        rt2 = math.sqrt(2)
        ctx.begin_path()
        ctx.move_to(mid_x - 0.5 * mark_size / rt2, mid_y - 0.5 * mark_size / rt2)
        ctx.line_to(mid_x + 0.5 * mark_size / rt2, mid_y + 0.5 * mark_size / rt2)
        ctx.move_to(mid_x - 0.5 * mark_size / rt2, mid_y + 0.5 * mark_size / rt2)
        ctx.line_to(mid_x + 0.5 * mark_size / rt2, mid_y - 0.5 * mark_size / rt2)
        ctx.stroke_style = stroke_style
        ctx.stroke()
    else:
        if graphic_size[0] > mark_size:
            ctx.begin_path()
            ctx.move_to(mid_x - 0.5 * mark_size, mid_y)
            ctx.line_to(mid_x + 0.5 * mark_size, mid_y)
            ctx.stroke_style = stroke_style
            ctx.stroke()
        if graphic_size[1] > mark_size:
            ctx.begin_path()
...
def draw_rect_marker(ctx: DrawingContextLike, r: Geometry.FloatRect, is_enabled: bool, is_shape_locked: bool) -> None:
    draw_marker(ctx, r.top_left, is_enabled, is_shape_locked)
    draw_marker(ctx, r.top_right, is_enabled, is_shape_locked)
    ...
    if is_selected:
        with ctx.saver():
            if rotation:
                ...
                ctx.rotate(-rotation)
                ctx.translate(-center.x, -center.y)
            draw_rect_marker(ctx, rect, is_focused, is_shape_locked)
            draw_centre_marker(ctx, rect.center, size, stroke_style, is_position_locked)
...
class Graphic(Persistence.PersistentObject):
    ...
    def draw(self, ctx: DrawingContextLike, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, mapping: CoordinateMappingLike, is_selected: bool, is_focused: bool) -> None:
        if not self._closed:
            self.get_renderer().draw(ctx, drawing_metrics, display_style, mapping, is_selected, is_focused)

    def get_renderer(self) -> GraphicRenderer:
        raise NotImplementedError()

    def test(self, mapping: CoordinateMappingLike, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, p: Geometry.FloatPoint, move_only: bool) -> typing.Tuple[typing.Optional[str], bool]:
        raise NotImplementedError()
    ...
    def label_position(self, drawing_metrics: UISettings.DrawingMetrics, mapping: CoordinateMappingLike, scaled_text_size: Geometry.FloatSize, scaled_padding: float) -> typing.Optional[Geometry.FloatPoint]:
        return self.get_renderer().label_position(drawing_metrics, mapping, scaled_text_size, scaled_padding)

    def reset_position(self) -> None:
        pass

    def test_label(self, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, mapping: CoordinateMappingLike, test_point: Geometry.FloatPoint) -> bool:
        if self.label:
            scaled_padding = drawing_metrics.scale_length(4.0)
            font = display_style.get_font("graphic-label", drawing_metrics)
            font_metrics = drawing_metrics.get_font_metrics(font, self.label)
            scaled_text_size = Geometry.FloatSize(height=font_metrics.height, width=font_metrics.width)
            text_pos = self.label_position(drawing_metrics, mapping, scaled_text_size, scaled_padding)
            if text_pos is not None:
                ...
                return test_inside_bounds(bounds, test_point, drawing_metrics.cursor_tolerance)
        return False
...
class GraphicRenderer:
    ...
    Represents an immutable snapshot of the properties of a Graphic that can be used for drawing and change detection.

    The display pipeline needs to be able to compare the current state of a graphic with the previous state to
    determine whether it needs to be redrawn. This can be done using the graphic_uuid and modified_count properties.

    The display pipeline and particularly the canvas item composers render in a thread and require a snapshot of the
    graphic state that is guaranteed not to change while drawing.

    Provides a draw method that can be called by the canvas when drawing graphics, and a label_position method that can
    be used to determine the position of the graphic label.
    """
    ...
    def draw_label(self, ctx: DrawingContextLike, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, mapping: CoordinateMappingLike, is_selected: bool) -> None:
        label = self.label
        if label and self.is_label_visible(Graphic.resolve_used_property(self.used_text_visibility_map, "label_text"), is_selected):
            scaled_padding = drawing_metrics.scale_length(4.0)
            font = display_style.get_font("graphic-label", drawing_metrics)
            font_metrics = drawing_metrics.get_font_metrics(font, label)
            scaled_text_size = Geometry.FloatSize(height=font_metrics.height, width=font_metrics.width)
            text_pos = self.label_position(drawing_metrics, mapping, scaled_text_size, scaled_padding)
            if text_pos:
                with ctx.saver():
                    ...
                    ctx.line_width = drawing_metrics.scale_stroke(1.0)
...
class MissingGraphic(Graphic):
    ...
    def get_renderer(self) -> GraphicRenderer:
        return MissingGraphicRenderer(self)
...
class RectangleTypeGraphic(Graphic):
    ...
    def test(self, mapping: CoordinateMappingLike, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, p: Geometry.FloatPoint, move_only: bool) -> typing.Tuple[typing.Optional[str], bool]:
        ...
        part, specific = test_rectangle(p, drawing_metrics.cursor_tolerance, center, size, rotation)
        if part is not None:
            return part, specific

        # label
        if self.test_label(drawing_metrics, display_style, mapping, p):
            return "all", False
...
class RectangleTypeGraphicRenderer(GraphicRenderer):

    def __init__(self, graphic: RectangleTypeGraphic) -> None:
        super().__init__(graphic)
        self.bounds = graphic.bounds
        self.rotation = graphic.rotation
...
class RectangleGraphic(RectangleTypeGraphic):
    ...
    def get_renderer(self) -> GraphicRenderer:
        return RectangleGraphicRenderer(self)
...
class RectangleGraphicRenderer(RectangleTypeGraphicRenderer):
    ...
    def draw(self, ctx: DrawingContextLike, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, mapping: CoordinateMappingLike, is_selected: bool, is_focused: bool) -> None:
        ...
        scaled_stroke_width = drawing_metrics.scale_stroke(used_stroke_width)
        orientation_stroke_width = drawing_metrics.scale_stroke(1.0)
        orientation_arrow_outer_offset = drawing_metrics.scale_length(10.0)
        orientation_arrow_inner_offset = drawing_metrics.scale_length(2.0)
        orientation_arrow_size = drawing_metrics.scale_length(4.0)
        ...
        if is_selected:
            with ctx.saver():
                if rotation:
                    ...
                    ctx.translate(-center.x, -center.y)
                draw_rect_marker(ctx, rect, is_focused, is_shape_locked)
                draw_centre_marker(ctx, center, rect.size, used_stroke_style, is_position_locked)
                # draw rotation marker
                top_middle = Geometry.FloatPoint(y=rect.top, x=rect.center.x)
                rotation_point = extend_line(center, top_middle, 14)
                ctx.begin_path()
                ...
                ctx.stroke()
                draw_circular_marker(ctx, rotation_point, is_focused, is_rotation_locked)
        self.draw_label(ctx, drawing_metrics, display_style, mapping, is_selected)
...
class EllipseGraphic(RectangleTypeGraphic):
    ...
    def get_renderer(self) -> GraphicRenderer:
        return EllipseGraphicRenderer(self)
...
class LineTypeGraphic(Graphic):
    ...
    def test(self, mapping: CoordinateMappingLike, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, p: Geometry.FloatPoint, move_only: bool) -> typing.Tuple[typing.Optional[str], bool]:
        ...
        # start point
        if test_point(p1, p, drawing_metrics.cursor_tolerance):
            return "start", True
        # end point
        if test_point(p2, p, drawing_metrics.cursor_tolerance):
            return "end", True
        # along the line
        if test_line(p1, p2, p, drawing_metrics.cursor_tolerance):
            return "all", True
        # label
        if self.test_label(drawing_metrics, display_style, mapping, p):
            return "all", False
    ...
    def adjust_part(self, mapping: CoordinateMappingLike, original: Geometry.FloatPoint, current: Geometry.FloatPoint, part: DragPartDataPlus, modifiers: ModifiersLike) -> None:
        ...
        pivot_image = mapping.map_point_image_norm_to_image(pivot)

        if not "shape" in constraints and part[0] in {"start", "end"}:
            dy = p_image.y - pivot_image.y
            dx = p_image.x - pivot_image.x
            if modifiers.shift:
                angle_degrees = math.degrees(math.atan2(abs(dy), abs(dx)))
                if angle_degrees > 60:
                    p_image = Geometry.FloatPoint(p_image.y, pivot_image.x)
                elif angle_degrees > 30:
                    if angle_degrees > 45:
                        if dx * dy > 0:
                            p_image = Geometry.FloatPoint(p_image.y, pivot_image.x + dy)
                        else:
                            p_image = Geometry.FloatPoint(p_image.y, pivot_image.x - dy)
                    else:
                        if dx * dy > 0:
                            p_image = Geometry.FloatPoint(pivot_image.y + dx, p_image.x)
                        else:
                            p_image = Geometry.FloatPoint(pivot_image.y - dx, p_image.x)
                else:
                    p_image = Geometry.FloatPoint(pivot_image.y, p_image.x)
...
class LineTypeGraphicRenderer(GraphicRenderer):

    def __init__(self, graphic: LineTypeGraphic) -> None:
        super().__init__(graphic)
...
class LineGraphic(LineTypeGraphic):
    ...
    def get_renderer(self) -> GraphicRenderer:
        return LineGraphicRenderer(self)
...
class LineGraphicRenderer(LineTypeGraphicRenderer):
    ...
    def draw(self, ctx: DrawingContextLike, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, mapping: CoordinateMappingLike, is_selected: bool, is_focused: bool) -> None:
        ...
        used_stroke_width = self.used_stroke_width
        scaled_stroke_width = drawing_metrics.scale_stroke(used_stroke_width)
        arrow_size = drawing_metrics.scale_length(8.0)
        used_stroke_style = self.used_stroke_style
        is_shape_locked = self.is_shape_locked
...
class LineProfileGraphic(LineTypeGraphic):
    ...
    def get_renderer(self) -> GraphicRenderer:
        return LineProfileGraphicRenderer(self)


class LineProfileGraphicRenderer(LineTypeGraphicRenderer):

    def __init__(self, graphic: LineProfileGraphic) -> None:
        super().__init__(graphic)
        ...
        with ctx.saver():
            ...
            if w > 1.0:
                ...
                with ctx.saver():
                    ...
                    ctx.close_path()
                    ctx.line_width = scaled_stroke_width
                    ctx.line_dash = int(drawing_metrics.scale_length(2.0))
                    ctx.stroke_style = used_stroke_style
                    ctx.stroke()
            for interval_descriptor in interval_descriptors:
                ...
                interval_marker_half_width = drawing_metrics.scale_length(4)
                if interval:
                    with ctx.saver():
...
class PointTypeGraphic(Graphic):
    ...
    def test(self, mapping: CoordinateMappingLike, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, p: Geometry.FloatPoint, move_only: bool) -> typing.Tuple[typing.Optional[str], bool]:
        ...
        if test_inside_bounds(bounds, p, drawing_metrics.cursor_tolerance):
            return "all", True
        # check the label
        if self.test_label(drawing_metrics, display_style, mapping, p):
            return "all", False
...
class PointTypeGraphicRenderer(GraphicRenderer):

    def __init__(self, graphic: PointTypeGraphic) -> None:
        super().__init__(graphic)
...
class PointGraphic(PointTypeGraphic):
    ...
    def get_renderer(self) -> GraphicRenderer:
        return PointGraphicRenderer(self)


class PointGraphicRenderer(PointTypeGraphicRenderer):

    def __init__(self, graphic: PointGraphic) -> None:
        super().__init__(graphic)
        self.cross_hair_size = graphic.cross_hair_size

    def draw(self, ctx: DrawingContextLike, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, mapping: CoordinateMappingLike, is_selected: bool, is_focused: bool) -> None:
        position = self.position
        scaled_cross_hair_size = drawing_metrics.scale_stroke(self.cross_hair_size)
        used_stroke_width = self.used_stroke_width
        scaled_stroke_width = drawing_metrics.scale_stroke(used_stroke_width)
        used_stroke_style = self.used_stroke_style
        ...
        with ctx.saver():
            ctx.begin_path()
            inner_size = drawing_metrics.scale_stroke(4)
    ...
    def label_position(self, drawing_metrics: UISettings.DrawingMetrics, mapping: CoordinateMappingLike, scaled_text_size: Geometry.FloatSize, scaled_padding: float) -> typing.Optional[Geometry.FloatPoint]:
        ...
        scaled_cross_hair_size = drawing_metrics.scale_length(self.cross_hair_size)
...
class IntervalGraphic(Graphic):
    ...
    def test(self, mapping: CoordinateMappingLike, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, p: Geometry.FloatPoint, move_only: bool) -> typing.Tuple[typing.Optional[str], bool]:
        ...
        # start point
        if abs(p.x - p1) < drawing_metrics.cursor_tolerance:
            return "start", True
        # end point
        if abs(p.x - p2) < drawing_metrics.cursor_tolerance:
            return "end", True
        # along the line
        if p.x > p1 - drawing_metrics.cursor_tolerance and p.x < p2 + drawing_metrics.cursor_tolerance:
            return "all", False
        # label
        if self.test_label(drawing_metrics, display_style, mapping, p):
            return "all", False
...
class IntervalGraphicRenderer(GraphicRenderer):

    def __init__(self, graphic: IntervalGraphic) -> None:
        super().__init__(graphic)
        self.interval = graphic.interval
        self.start = graphic.start
        self.end = graphic.end
...
class ChannelGraphic(Graphic):
    ...
    def test(self, mapping: CoordinateMappingLike, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, p: Geometry.FloatPoint, move_only: bool) -> typing.Tuple[typing.Optional[str], bool]:
        ...
        if abs(p.x - pos) < drawing_metrics.cursor_tolerance:
            return "all", True
        # label
        if self.test_label(drawing_metrics, display_style, mapping, p):
            return "all", False
...
class ChannelGraphicRenderer(GraphicRenderer):

    def __init__(self, graphic: ChannelGraphic) -> None:
        super().__init__(graphic)
        self.position = graphic.position
...
class SpotGraphic(Graphic):
    ...
    def test(self, mapping: CoordinateMappingLike, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, p: Geometry.FloatPoint, move_only: bool) -> typing.Tuple[typing.Optional[str], bool]:
        ...
        part, specific = test_rectangle(p, drawing_metrics.cursor_tolerance, center, size, rotation + math.pi)
        if part is not None:
            if part == "top-left":
...
class SpotGraphicRenderer(GraphicRenderer):

    def __init__(self, graphic: SpotGraphic) -> None:
        super().__init__(graphic)
        self.bounds = graphic.bounds
        self.rotation = graphic.rotation
...
class WedgeGraphic(Graphic):
    ...
    def test(self, mapping: CoordinateMappingLike, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, p: Geometry.FloatPoint, move_only: bool) -> typing.Tuple[typing.Optional[str], bool]:
        ...
        if test_line(center, start_line_endpoint, p, drawing_metrics.cursor_tolerance):
            return "start-angle", True
        if test_line(center, end_line_endpoint, p, drawing_metrics.cursor_tolerance):
            return "end-angle", True
        if test_line(center, start_line_endpoint_inverted, p, drawing_metrics.cursor_tolerance):
            return "inverted-start-angle", True
        if test_line(center, end_line_endpoint_inverted, p, drawing_metrics.cursor_tolerance):
    ...
    def get_renderer(self) -> GraphicRenderer:
        return WedgeGraphicRenderer(self)


class WedgeGraphicRenderer(GraphicRenderer):

    def __init__(self, graphic: WedgeGraphic) -> None:
        super().__init__(graphic)
...
class RingGraphic(Graphic):
    ...
    def __init__(self) -> None:
        ...
        self.title = _("Band-Pass Graphic")
    ...
    def test(self, mapping: CoordinateMappingLike, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, p: Geometry.FloatPoint, move_only: bool) -> typing.Tuple[typing.Optional[str], bool]:
        ...
        if self.mode == "band-pass" or self.mode == "high-pass":
            if test_point(top_marker_outer, p, drawing_metrics.cursor_tolerance):
                return "radius_1", True
            if test_point(bottom_marker_outer, p, drawing_metrics.cursor_tolerance):
                return "radius_1", True
            if test_point(left_marker_outer, p, drawing_metrics.cursor_tolerance):
                return "radius_1", True
            if test_point(right_marker_outer, p, drawing_metrics.cursor_tolerance):
                return "radius_1", True
        if self.mode == "band-pass" or self.mode == "low-pass":
            if test_point(top_marker_inner, p, drawing_metrics.cursor_tolerance):
                return "radius_2", True
            if test_point(bottom_marker_inner, p, drawing_metrics.cursor_tolerance):
                return "radius_2", True
            if test_point(left_marker_inner, p, drawing_metrics.cursor_tolerance):
                return "radius_2", True
            if test_point(right_marker_inner, p, drawing_metrics.cursor_tolerance):
                return "radius_2", True
    ...
    def get_renderer(self) -> GraphicRenderer:
        return RingGraphicRenderer(self)


class RingGraphicRenderer(GraphicRenderer):

    def __init__(self, graphic: RingGraphic) -> None:
        super().__init__(graphic)
        self.radius_1 = graphic.radius_1
    ...
    def draw(self, ctx: DrawingContextLike, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, mapping: CoordinateMappingLike, is_selected: bool, is_focused: bool) -> None:
        ...
        radius_1_widget = mapping.map_size_image_norm_to_widget(Geometry.FloatSize(radius_1, radius_1))
        radius_2_widget = mapping.map_size_image_norm_to_widget(Geometry.FloatSize(radius_2, radius_2))
        with ctx.saver():
            if not mode == "low-pass":
                ctx.line_width = scaled_stroke_width
                ctx.stroke_style = used_stroke_style
                draw_ellipse(ctx, center,
                             Geometry.FloatSize(width=radius_1_widget[1] * 2, height=radius_1_widget[0] * 2),
                             used_stroke_style, None)
                if is_selected:
                    draw_marker(ctx, Geometry.FloatPoint(center.y + radius_1_widget[0], center.x), is_focused, is_shape_locked)
                    draw_marker(ctx, Geometry.FloatPoint(center.y - radius_1_widget[0], center.x), is_focused, is_shape_locked)
                    draw_marker(ctx, Geometry.FloatPoint(center.y, center.x + radius_1_widget[1]), is_focused, is_shape_locked)
                    draw_marker(ctx, Geometry.FloatPoint(center.y, center.x - radius_1_widget[1]), is_focused, is_shape_locked)
            if not mode == "high-pass":
                ctx.line_width = scaled_stroke_width
                ctx.stroke_style = used_stroke_style
                draw_ellipse(ctx, center, Geometry.FloatSize(width=radius_2_widget[1] * 2, height=radius_2_widget[0] * 2), used_stroke_style, None)
                if is_selected:
                    draw_marker(ctx, Geometry.FloatPoint(center.y + radius_2_widget[0], center.x), is_focused, is_shape_locked)
                    draw_marker(ctx, Geometry.FloatPoint(center.y - radius_2_widget[0], center.x), is_focused, is_shape_locked)
                    draw_marker(ctx, Geometry.FloatPoint(center.y, center.x + radius_2_widget[1]), is_focused, is_shape_locked)
                    draw_marker(ctx, Geometry.FloatPoint(center.y, center.x - radius_2_widget[1]), is_focused, is_shape_locked)
            # draw 2 thick arcs
            ctx.fill_style = used_fill_style
...
class LatticeGraphic(Graphic):
    ...
    def test(self, mapping: CoordinateMappingLike, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, p: Geometry.FloatPoint, move_only: bool) -> typing.Tuple[typing.Optional[str], bool]:
        ...
        # test u, v centers
        if test_point(u_bounds.center, p, drawing_metrics.cursor_tolerance):
            return "u-all", True
        if test_point(v_bounds.center, p, drawing_metrics.cursor_tolerance):
            return "v-all", True

        # test u-corners
        if test_point(u_bounds.top_left, p, drawing_metrics.cursor_tolerance):
            return "u-top-left", True
        if test_point(u_bounds.top_right, p, drawing_metrics.cursor_tolerance):
            return "u-top-right", True
        if test_point(u_bounds.bottom_left, p, drawing_metrics.cursor_tolerance):
            return "u-bottom-left", True
        if test_point(u_bounds.bottom_right, p, drawing_metrics.cursor_tolerance):
            return "u-bottom-right", True

        # test v-corners
        if test_point(v_bounds.top_left, p, drawing_metrics.cursor_tolerance):
            return "v-top-left", True
        if test_point(v_bounds.top_right, p, drawing_metrics.cursor_tolerance):
            return "v-top-right", True
        if test_point(v_bounds.bottom_left, p, drawing_metrics.cursor_tolerance):
            return "v-bottom-left", True
        if test_point(v_bounds.bottom_right, p, drawing_metrics.cursor_tolerance):
            return "v-bottom-right", True

        # test u-boundary
        if test_line(u_bounds.top_left, u_bounds.top_right, p, drawing_metrics.cursor_tolerance):
            return "u-all", True
        if test_line(u_bounds.bottom_left, u_bounds.bottom_right, p, drawing_metrics.cursor_tolerance):
            return "u-all", True
        if test_line(u_bounds.top_left, u_bounds.bottom_left, p, drawing_metrics.cursor_tolerance):
            return "u-all", True
        if test_line(u_bounds.top_right, u_bounds.bottom_right, p, drawing_metrics.cursor_tolerance):
            return "u-all", True

        # test v-boundary
        if test_line(v_bounds.top_left, v_bounds.top_right, p, drawing_metrics.cursor_tolerance):
            return "v-all", True
        if test_line(v_bounds.bottom_left, v_bounds.bottom_right, p, drawing_metrics.cursor_tolerance):
            return "v-all", True
        if test_line(v_bounds.top_left, v_bounds.bottom_left, p, drawing_metrics.cursor_tolerance):
            return "v-all", True
        if test_line(v_bounds.top_right, v_bounds.bottom_right, p, drawing_metrics.cursor_tolerance):
            return "v-all", True

        # test u, v interiors
        if test_inside_bounds(u_bounds, p, drawing_metrics.cursor_tolerance):
            return "u-all", True
        if test_inside_bounds(v_bounds, p, drawing_metrics.cursor_tolerance):
            return "v-all", True

        # start point
        if test_point(start, p, drawing_metrics.cursor_tolerance):
            return "all", True

        # along the lines
        if test_line(start, u_end, p, drawing_metrics.cursor_tolerance):
            return "all", True
        if test_line(start, v_end, p, drawing_metrics.cursor_tolerance):
            return "all", True
    ...
    def get_renderer(self) -> GraphicRenderer:
        return LatticeGraphicRenderer(self)


class LatticeGraphicRenderer(GraphicRenderer):

    def __init__(self, graphic: LatticeGraphic) -> None:
        super().__init__(graphic)
        self.u_pos = graphic.u_pos
    ...
    def draw(self, ctx: DrawingContextLike, drawing_metrics: UISettings.DrawingMetrics, display_style: UISettings.DisplayStyle, mapping: CoordinateMappingLike, is_selected: bool, is_focused: bool) -> None:
        ...
        with ctx.saver():
            ...
            ctx.stroke()
            ctx.fill_style = used_fill_style
            draw_ellipse(ctx, u_pos_widget, size_widget, used_stroke_style, used_fill_style)
        with ctx.saver():
            ctx.begin_path()
            ctx.move_to(start_widget[1], start_widget[0])
            ...
            ctx.stroke()
            ctx.fill_style = used_fill_style
            draw_ellipse(ctx, v_pos_widget, size_widget, used_stroke_style, used_fill_style)
                    ...
                    for vi in range(-mx, mx + 1):
                        ...
                        if ui == -mx or ui == mx or vi == -mx or vi == mx:
                            ...
                            if bounds.contains_point(p):
                                ...
                                draw_ellipse(ctx, p_widget, size_widget, used_stroke_style, used_fill_style)
                                drawn = True
                mx += 1

        if is_selected:
            draw_marker(ctx, start_widget, is_focused, is_position_locked)
            draw_marker(ctx, u_pos_widget, is_focused, is_position_locked)
...
class RectangleMaskItem(MaskItem):
...
class EllipseMaskItem(MaskItem):
    def __init__(self, bounds: Geometry.FloatRect, rotation: float) -> None:
        self.bounds = bounds
        self.rotation = rotation
...
class LatticeMaskItem(MaskItem):
