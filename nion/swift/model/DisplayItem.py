...
class DisplayPropertiesLayersGraphicsStream(Stream.ValueStream[DisplayPropertiesLayersGraphics]):
    def __init__(self, display_item: DisplayItem) -> None:
        ...
        self.__graphic_selection_changed_event_listener = display_item.graphic_selection_changed_event.listen(ReferenceCounting.weak_partial(self.__class__.__graphic_selection_changed, self))
        self.__graphic_changed_event_listener = display_item.graphics_changed_event.listen(ReferenceCounting.weak_partial(self.__class__.__graphics_changed, self))
    ...
    def __display_item_item_inserted(self, key: str, item: typing.Any, index: int) -> None:
        if key == "graphics":
            ...
            self.__display_layer_changed_listeners.insert(index, item.property_changed_event.listen(ReferenceCounting.weak_partial(self.__class__.__display_layer_changed, self)))
            self.__send_delta()
    ...
    def display_layers_list(self) -> list[DisplayLayer]:
        ...
        for display_layer in self.display_layers:
            ...
            if display_layer.display_data_channel and display_layer.display_data_channel in self.display_data_channels:
                display_layer_c.data_index = self.display_data_channels.index(display_layer.display_data_channel)
            display_layer_list.append(display_layer_c)
        return display_layer_list
    ...
    def display_layers_list(self, value: list[DisplayLayer]) -> None:
        ...
        for index, (display_layer, new_display_layer) in enumerate(zip(self.display_layers, value)):
            for property in properties:
                ...
                self._set_display_layer_property(index, property, property_value)
