...
class TestDisplayPanelClass(unittest.TestCase):
    ...
    def test_stacked_lineplot_interval_zoom_with_different_intensity_calibration_scales(self):
        with TestContext.create_memory_context() as test_context:
            document_controller = test_context.create_document_controller()
            document_model = document_controller.document_model
            data1 = numpy.array([x for x in range(-5, 6)]) * 1
            ...
            data2 = numpy.array([x for x in range(-5, 6)]) * 2
    ...
    def test_stacked_lineplot_interval_zoom_with_different_intensity_calibrations_with_no_selected_interval(self):
        with TestContext.create_memory_context() as test_context:
            ...
            display_panel.display_canvas_item.prepare_display()  # force layout
            axes = line_plot_canvas_item._axes
            self.assertAlmostEqual(axes.drawn_left_channel, ceil(0 - 11*.5))
            self.assertAlmostEqual(axes.drawn_right_channel, floor(11 * 1.5))
            self.assertAlmostEqual(5 * 2 * 8 * 1.2, axes.uncalibrated_data_max)
            self.assertAlmostEqual(-5 * 2 * 8 * 1.2, axes.uncalibrated_data_min)
    ...
    def test_stacked_lineplot_interval_zoom_with_different_intensity_calibrations_with_intensity_offset(self):
        with TestContext.create_memory_context() as test_context:
            ...
            data2 = numpy.array([x for x in range(-5, 6)]) * 1
            data_item2 = DataItem.DataItem(data2)
    ...
    def test_stacked_lineplot_interval_zoom_with_different_intensity_calibrations_with_multiple_intervals(self):
        with TestContext.create_memory_context() as test_context:
            ...
            self.assertAlmostEqual(13 * 2 * 1.2, axes.uncalibrated_data_max)
            self.assertAlmostEqual(0, axes.uncalibrated_data_min)  # since no negative values, our min point is 0
