...
class TestGraphicsClass(unittest.TestCase):
    ...
    def test_rect_test(self):
        ...
        self.assertEqual(rect_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((500, 500)), move_only=False), ("all", False))
        self.assertEqual(rect_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((250, 250)), move_only=False)[0], "top-left")
        self.assertEqual(rect_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((750, 750)), move_only=False)[0], "bottom-right")
        self.assertEqual(rect_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((250, 750)), move_only=False)[0], "top-right")
        self.assertEqual(rect_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((750, 250)), move_only=False)[0], "bottom-left")
        self.assertEqual(rect_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((250, 500)), move_only=False), ("all", True))
        self.assertEqual(rect_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((750, 500)), move_only=False), ("all", True))
        self.assertEqual(rect_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((500, 250)), move_only=False), ("all", True))
        self.assertEqual(rect_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((500, 750)), move_only=False), ("all", True))
        self.assertIsNone(rect_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint(), move_only=False)[0])
        rect_graphic.close()
    ...
    def test_line_test(self):
        ...
        drawing_metrics = UISettings.DrawingMetrics(ui_settings=ui_settings, ppi=96.0)
        display_style = UISettings.DisplayStyle()
        self.assertEqual(line_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((500, 500)), move_only=True)[0], "all")
        self.assertEqual(line_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((250, 250)), move_only=True)[0], "start")
        self.assertEqual(line_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((750, 750)), move_only=True)[0], "end")
        self.assertEqual(line_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((250, 250)), move_only=False)[0], "start")
        self.assertEqual(line_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((750, 750)), move_only=False)[0], "end")
        self.assertIsNone(line_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((240, 240)), move_only=False)[0])
        self.assertIsNone(line_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((760, 760)), move_only=False)[0])
        self.assertIsNone(line_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint(), move_only=False)[0])
        line_graphic.close()
    ...
    def test_line_graphic_position_locked(self):
        with TestContext.create_memory_context() as test_context:
            document_controller = test_context.create_document_controller()
            document_model = document_controller.document_model
            display_panel = document_controller.selected_display_panel
            ...
            original_start = Geometry.FloatPoint.make(line_graphic.start)
            original_end = Geometry.FloatPoint.make(line_graphic.end)
            original_midpoint = Geometry.midpoint(original_start, original_end)
            #Test 1- Attempt to drag the whole line — should be ignored
            display_panel.display_canvas_item.simulate_drag((500, 500), (600, 600))
            document_controller.periodic()
            self.assertAlmostEqualPoint(original_start, line_graphic.start)
            self.assertAlmostEqualPoint(original_end, line_graphic.end)
            #Test 2- Drag start point — should work
            display_panel.display_canvas_item.simulate_drag((250, 250), (300, 300))
            document_controller.periodic()
            self.assertNotEqual(line_graphic.start, original_start)
            # Midpoint should remain unchanged
            new_midpoint = Geometry.midpoint(line_graphic.start, line_graphic.end)
            self.assertAlmostEqualPoint(original_midpoint, new_midpoint)
            #Test 3- Drag end point — should work
            display_panel.display_canvas_item.simulate_drag((750, 750), (700, 700))
            document_controller.periodic()
            self.assertNotEqual(line_graphic.end, original_end)
            # Midpoint should still remain unchanged
    ...
    def test_spot_test(self):
        ...
        drawing_metrics = UISettings.DrawingMetrics(ui_settings=ui_settings, ppi=96.0)
        display_style = UISettings.DisplayStyle()
        self.assertEqual(spot_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((200, 200)), move_only=False), ("top-left", True))
        self.assertEqual(spot_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((300, 200)), move_only=False), ("bottom-left", True))
        self.assertEqual(spot_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((300, 300)), move_only=False), ("bottom-right", True))
        self.assertEqual(spot_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((200, 300)), move_only=False), ("top-right", True))
        self.assertEqual(spot_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((185, 250)), move_only=False), ("rotate", True))
        self.assertEqual(spot_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((800, 800)), move_only=False), ("inverted-bottom-right", True))
        self.assertEqual(spot_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((700, 800)), move_only=False), ("inverted-top-right", True))
        self.assertEqual(spot_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((700, 700)), move_only=False), ("inverted-top-left", True))
        self.assertEqual(spot_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((800, 700)), move_only=False), ("inverted-bottom-left", True))
        self.assertEqual(spot_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint.make((815, 750)), move_only=False), ("inverted-rotate", True))
        self.assertIsNone(spot_graphic.test(mapping, drawing_metrics, display_style, Geometry.FloatPoint(), move_only=False)[0])
        spot_graphic.close()
    ...
    def test_changing_data_length_does_not_update_graphics(self):
        with TestContext.create_memory_context() as test_context:
            ...
            self.assertEqual(display_item.display_info.display_calibration_info.displayed_dimensional_scales[-1], 100)
