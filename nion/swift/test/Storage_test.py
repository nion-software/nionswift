    ...
    def test_reloaded_line_plot_has_valid_calibration_style(self):
        with create_memory_profile_context() as profile_context:
            ...
            with document_model.ref():
                ...
                self.assertEqual("calibrated", display_item.display_info.display_calibration_info.calibration_style_id)
            # read it back
            document_model = profile_context.create_document_model(auto_close=False)
            with document_model.ref():
                data_item = document_model.data_items[0]
    ...
    def test_reloading_composite_operation_reconnects_when_reloaded(self):
        with create_memory_profile_context() as profile_context:
            ...
            with contextlib.closing(document_controller):
                read_data_item = document_model.data_items[0]
                read_display_item = document_model.get_display_item_for_data_item(read_data_item)
                computation_bounds = document_model.get_data_item_computation(document_model.data_items[1]).get_input_graphic("src").bounds
    ...
    def test_data_items_v8_to_v9_cross_correlate_migration(self):
        ...
        with create_memory_profile_context() as profile_context:
            ...
            with document_model.ref():
                ...
                self.assertEqual(computation.get_input_data_item("src1"), document_model.data_items[0])
                self.assertEqual(computation.get_input_data_item("src2"), document_model.data_items[1])
    ...
    def test_data_items_v10_to_v11_cross_correlate_migration(self):
        with create_memory_profile_context() as profile_context:
            ...
            with document_model.ref():
                ...
                self.assertEqual(len(computation.variables), 2)
                self.assertEqual(computation.get_input_data_item("src1"), document_model.data_items[0])
                self.assertEqual(computation.get_input_graphic("src1"), document_model.display_items[0].graphics[0])
                self.assertEqual(computation.get_input_data_item("src2"), document_model.data_items[1])
                self.assertEqual(computation.get_input_graphic("src2"), document_model.display_items[1].graphics[0])
                self.assertEqual(len(computation.variables), 2)
                data1 = numpy.arange(64).reshape((8, 8))
                document_model.data_items[0].set_data(data1)
                data2 = numpy.arange(64).reshape((8, 8))
