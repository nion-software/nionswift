...
class TestProcessingClass(unittest.TestCase):
    ...
    def test_basic_redimension_command_works(self):
        with TestContext.create_memory_context() as test_context:
            ...
            self.assertEqual(redim_data_item.data_descriptor, DataAndMetadata.DataDescriptor(False, 1, 2))
            self.assertEqual(list(redim_data_item.dimensional_calibrations), [Calibration.Calibration(1), Calibration.Calibration(2), Calibration.Calibration(3)])
    ...
    def test_squeeze_removes_proper_dimensions(self):
        with TestContext.create_memory_context() as test_context:
            ...
            self.assertEqual(squeezed_data_item.data_descriptor, DataAndMetadata.DataDescriptor(False, 1, 1))
            self.assertEqual(list(squeezed_data_item.dimensional_calibrations), [Calibration.Calibration(2), Calibration.Calibration(4)])
    ...
    def test_squeeze_removes_proper_dimensions_when_both_datum_is_1(self):
        with TestContext.create_memory_context() as test_context:
            ...
            squeezed_data_item = document_model.data_items[1]
            self.assertEqual(squeezed_data_item.data_descriptor, DataAndMetadata.DataDescriptor(False, 1, 1))
            self.assertEqual(list(squeezed_data_item.dimensional_calibrations), [Calibration.Calibration(2), Calibration.Calibration(5)])
