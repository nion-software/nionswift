...
class LineGraphRegionsCanvasItemComposer(CanvasItem.BaseComposer):
    ...
    def _repaint(self, drawing_context: DrawingContext.DrawingContext, canvas_bounds: Geometry.IntRect, composer_cache: CanvasItem.ComposerCache) -> None:
        ...
        if axes:
            ...
            def get_text_rectangle(text: str, x: float, y: float, text_baseline: str) -> Geometry.FloatRect:
            ...
            def draw_text_blocked_line(x_pos: float, width_text_rect: Geometry.FloatRect | None, label_text_rect: Geometry.FloatRect | None) -> None:
                with drawing_context.saver():
                    drawing_context.move_to(x_pos, plot_origin_y)
                    if width_text_rect:
                        if width_text_rect.left < x_pos < width_text_rect.right:
                    ...
                    if label_text_rect:
                        if label_text_rect.left < x_pos < label_text_rect.right:
                ...
                with drawing_context.saver():
                    ...
                    if region_selected:
                        ...
                        if middle_text and region.style != "tag":
                            ...
                            mid_y = level - 15
                            mid_text_rect = get_text_rectangle(middle_text, mid_x, mid_y, baseline)
                    ...
                    if label:
                        ...
                        baseline = "top"
                        drawing_context.text_baseline = baseline
                        label_rect = get_text_rectangle(label, mid_x, label_y, baseline)
                        drawing_context.fill_text(label, mid_x, label_y)
                    draw_text_blocked_line(left, mid_text_rect, label_rect)
                    draw_text_blocked_line(right, mid_text_rect, label_rect)
                    drawing_context.close_path()
