...
class DataPanel(Panel.Panel):
    ...
    class DataItemsBinding(UserInterfaceUtility.Binding):
        ...
        def __init__(self):
            super(DataPanel.DataItemsBinding, self).__init__(None)
            ...
            with self._update_mutex:
                ...
                for data_item in data_items:
                    ...
                    if index < len(old_data_items) and old_data_items[index] not in data_items:
                        ...
                        self.queue_task(functools.partial(DataPanel.DataItemsBinding.__remove_item, self, data_item_to_remove, index))
                    ...
                    if data_item in old_data_items:
                        ...
                        if index < old_index:
                            self.queue_task(functools.partial(DataPanel.DataItemsBinding.__remove_item, self, data_item, old_index))
                            ...
                            self.queue_task(functools.partial(DataPanel.DataItemsBinding.__insert_item, self, data_item, index))
    ...
    class DataItemsFilterBinding(DataItemsBinding):
        ...
        def __init__(self):
            super(DataPanel.DataItemsFilterBinding, self).__init__()
    ...
    class DataItemsInContainerBinding(DataItemsBinding):
        ...
        def __init__(self):
            super(DataPanel.DataItemsInContainerBinding, self).__init__()
            self.__counted_data_items = collections.Counter()
            self.__container = None

        def close(self):
            super(DataPanel.DataItemsInContainerBinding, self).close()
            self.container = None
