    ...
    "axis_groups": [
      {
        ...
        "coordinate_calibrations": {
          "spatial": {
            "label": "Real-space (camera)",
            "calibrations": [
              {
                "scale": 0.1,
                "offset": 0.0,
...
- `descriptor`: The array descriptor from essential keys. Contains:
  ...
  - `intensity_calibrations`: A `CalibrationSet` for intensity (value) calibrations, with keyed calibrations and a primary key.
  - `intensity_calibrations.calibrations`: A dict from calibration key to calibration object (`scale`, `offset`, `unit`). Keys are free-form and case-sensitive.
  - `intensity_calibrations.names`: Optional dict from calibration key to human-readable display name for UI.
  - `intensity_calibrations.primary_key`: The key of the primary intensity calibration.
...
- `axis_groups[*]`: Each axis group contains:
  ...
  - `coordinate_calibrations`: A dict mapping keys (e.g., `"spatial"`, `"angular"`) to coordinate calibration objects.
  - `coordinate_calibrations[*].label`: A human-readable calibration label for UI and display.
  - `coordinate_calibrations[*].calibrations`: One calibration per axis in the group, ordered to match `axes`.
  - `primary_calibration_key`: The key of the primary coordinate calibration for this axis group.
...
Calibrations for axis coordinates are stored only in `coordinate_calibrations` at the axis-group level, not per axis. This makes coordinate-calibration keys group-scoped by construction and avoids per-axis key synchronization problems. Within one axis group, every key resolves to one coordinate calibration object containing a human-readable label and a complete per-axis calibration tuple, so a key always describes one coherent coordinate calibration for the whole group. For `intensity_calibrations`, keys are free-form for now; a constrained allowed-key set may be standardized later. Persistent serialization omits fields whose value is `null`.
